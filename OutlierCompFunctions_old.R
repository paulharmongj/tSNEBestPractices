...
MultiPermanova <- function(data, nperms = 5000, perp = 10, pca_options = FALSE){
  ...
  for(j in 1:n){
    ...
    x <- Rtsne(data_dist, perplexity = perp, pca = pca_options, is_distance = TRUE)
    maplist[[j]] <- x$Y
  }
  ...
  return(list(tsnelist = model_list, cmdlist = model_list_cmd))
}
