...
The below chunk runs the simulations. Each iteration simulates a new (contaminated) dataset, runs the t-SNE and MDS mapping procedure, and tests the consistency of the configurations for each of the *n* holdout points. 


```{r original_sims, eval = FALSE, cache = TRUE, warning = FALSE, message = FALSE}
...
for(j in 1:n_sims){
  ...
  ### CMD Plots #########################################################
  df3 = tibble(Holdout = 1:length(x3$cmdlist), PValues = sapply(x3$cmdlist, pullPval), Fstat = sapply(x3$cmdlist, pullFstat))
...
for(j in 1:n_sims){
  ...
  #### Simulate the Data with outliers adn a single group (simulates all from one group)
  
  g1 <- rmnorm(n = 10, mean = rep(20, 10), diag(45, 10)) %>%
    t() %>%
    data.frame()
  g2 <- rmnorm(n = 10, mean = rep(50, 20), diag(45, 20)) %>%
    t() %>%
    data.frame()
  g3 <- rmnorm(n = 10, mean = rep(80, 30), diag(45, 30)) %>%
    t() %>%
    data.frame()
